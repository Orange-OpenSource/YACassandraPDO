...
static pdo_cassandra_type pdo_cassandra_get_element_type_in_collection(const std::string &type)
{
   std::string real_type;
    ...
    if (!real_type.compare(0, 8, "ListType")) {
        real_type = real_type.substr(sizeof("ListType"), real_type.size() - sizeof("ListType") - 1);
        return pdo_cassandra_get_cassandra_type(real_type);
    }
...
zval* parse_collection(pdo_cassandra_type col_type, const std::string &type, const std::string &data) {
                ...
                case PDO_CASSANDRA_TYPE_SET:
					{
                        ...
                        zval *result = parse_collection(lparam_type, S->result.get ()->schema.value_types [current_column], (*col_it).value);
